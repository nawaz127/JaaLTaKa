...
def main():
    parser = argparse.ArgumentParser(description="Train MultiView Baseline Model")
    ...
    # Build model
    model = MultiViewBaseline(backbone_name=args.backbone, pretrained=True, freeze_backbone=False)
    total_params = sum(p.numel() for p in model.parameters())
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Parameters: {total_params:,} total, {trainable:,} trainable")

    exp_name = f"baseline_{args.backbone}"
